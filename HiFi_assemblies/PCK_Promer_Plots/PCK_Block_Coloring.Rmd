...
```{r}
blocks <- read_tsv("PCK_Blocks.tsv") %>%
...
AK2 <- combined_plot %>%
  filter((Query >= 21037211 & Query <= 24030820) |
...
# paths <- file.path(paste0("Promer_HiFi_vs_TAIR10_PCK_", 0:7, ".gplot"))
...
# Edit for plotting
```{bash}
<!-- for i in $(ls Promer_HiFi_vs_TAIR10_PCK_*.gplot) -->
<!-- do -->
...
```{r}
ACKs2 <- map2(all_plots, 0:7, ~.x %>% mutate(ACK = .y))
