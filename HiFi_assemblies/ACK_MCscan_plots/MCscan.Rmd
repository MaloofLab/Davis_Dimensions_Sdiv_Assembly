...
#Sdiv_ids <- read_tsv("1Mb_HiFiasm_S.div.fasta.fai", col_names = "seqID", col_select = 1)

Sdiv_ids <- c(3, 14, 2, 16, 15, 8, 18, 22, 1, 13, 4, 5, 9, 10, 7, 12, 11, 27) %>% 
...
Sdiv_ids1 <- c(3, 2, 8, 18, 11, 1, 16, 4, 9, 7, 27) %>% 
  str_pad(width=6, pad="0") %>% 
  str_c("ptg", ., "l")

Sdiv_ids1 <- Sdiv_ids1 %>% str_replace("((03|18)l)", "\\1-")

Sdiv_ids2 <- c(14, 15, 22, 13, 5, 10, 12) %>% 
  str_pad(width=6, pad="0") %>% 
  str_c("ptg", ., "l")

Sdiv_ids2 <- Sdiv_ids2 %>% str_replace("(1[025]l)", "\\1-")
